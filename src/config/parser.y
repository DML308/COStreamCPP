...
function.definition:
            type.specifier IDENTIFIER '(' ')' function.body{
                $$= new functionDclNode((primNode*)$1,$2,NULL,(funcBodyNode*)$5);
                line("Line:%-4d",@1.first_line);
                debug("function.definition::=%s\n",$$->toString().c_str());
            }
        |   type.specifier IDENTIFIER '('  parameter.list ')' function.body{
                $$=new functionDclNode((primNode*)$1,$2,$4,(funcBodyNode*)$6);
                line("Line:%-4d",@1.first_line);
                debug("function.definition::=%s\n",$$->toString().c_str());
            }
        ;
paramlist.list:
        ...
        |   parameter.list '=' initializer{
            yyerror("parameter.list in function definationd cannot have initializers\n");
            exit(-1);
            }
...
composite.body.statement.list:
        ...
        |   costream.body.statement.list costream.composite.statement   {if($2!=NULL) $$push_back($2);}
        ;
costream.composite.statement:
...
composite.body.operator:
            operator.file.writer    {
                line("Line:%-4d",@1.first_line);
                debug("composite.body.operator::=operator.flie.writer \n");
                $$=$1;
            }
        |   operator.add            {
            line("Line:%-4d",@1.first_line);
...
operator.splitjoin:
            SPLITJOIN lblock split.statement splitjoinPipeline.statement.list join.statement rblock {
                $$=new splitjoinPipeline(NULL,NULL,(splitNode*)$3,NULL,$4,(joinNode*)$5,@1);
            }
        |   SPLITJOIN lblock statement.list split.statement splitjoinPipeline.statement.list join.statement rblock  {
                $$=new splitjoinPipeline(NULL,NULL,(splitNode*)$4,$3,$5,(joinNode*)$6,@1);
            }
        ;
split.statement:
...
roundrobin.statement:
        ...
        |   ROUNDROBIN '(' exp ')' ';'  {$$=new roundrobinNode((expNode*)$3,@1);}
join.statement:
...
statement:
        ...
        |   compound.statement  {
            line("Line:%-4d",@1.first_line);
            debug("statement::=compound.statement(%s) \n");
...
labeled.statement:
            CASE exp ':' statement      {
                line("Line:%-4d",@1.first_line);
                debug("labled.statement::=CASE exp ':' statement \n");
                $$=new caseNode((expNode*)$2,$4,@3);
            }
        |   DEFAULT ':' statement       {
                line("Line:%-4d",@1.first_line);
                debug("labled.statement::=DEFAULT ':' statement \n");
                $$=new defaultNode($3,@2);
            }
        ;
compound.statement:
...
selection.statement:
        ...
        |   IF '(' exp ')' composite.composite.statement   
            ELSE costream.composite.statement               {
                                                                line("Line:%-4d",@1.first_line);
                                                                debug("selection.statement::=if(exp) costream.composite.statement esle……\n");
                                                                $$=new ifelseNode((expNode*)$3,$5,$7,@1);
                                                            }
        |   SWITCH '(' exp ')' statement                    {
                                                                $$=new switchNode((expNode*)$3,$5,$7,@1);
                                                            }
        ;
iteration.statement:
        ...
        |   FOR '(' exp ';' exp ';' 'exp' ')' costream.composite.statement  {
                line("Line:%-4d",@1.first_line);
                debug("iteratioin.statement::=for(……) costream.composite.statement \n");
...
exp:        idNode              {
        ...
        |   exp '?' exp ':' exp {$$=new ternary((expNode*)$1,(expNode*)$3,(expNode*)$5,@4);}
        ...
        |   exp DECR            {$$=new unaryNode("POSDEC",(expNode*)$1,@2);}
        |   '(' exp ')'         {$$=new parenNode((expNode*)$2,@2);}
        |   '(' basic.type.name ')' {$$=new castNode((primNode*)$2,(expNode*)$4,@3);}
        |   exp assignment.operator exp {
                                ...
                                if($3->type==SplitJoin){
                                    ...
                                    ((operatorNode*)$3)->outputs=new list<Node*>({$3});
                                }
                            }
        ...
        |   SPLITJOIN '(' argument.expression.list ')' lblock statement.split split.statement splitjoinPipeline.statement.list join.statement rblock {
                $$=new splitjoinNode($3,NULL,(splitNode*)$7,$6,$8,(joinNode*)$9,@1);
        |   PIPELINE   '(' arguexpressiont ')' lblock splitjoinPipeline.statement.list rblock {
...
operator.selfdefine.body:
            ...
            {
                line("Line:%-4d",@1.first_line);
                debug("operator.selfdefinr.body::={init work window.list}\n");
                $$=new operBodyNode(NULL,$2,$3,(windowNode*)$4);
            }
        |   lblock statement.list operator.selfdefine.body.init statement.list operator.selfdefine.body.work statement.list operator.selfdefine.body.window.list rblock
            {
                line("Line:%-4d",@1.first_line);
                debug("operator.selfdefinr.body::={statement.list init work window.list}\n");
                $$=new operBodyNode($2,$3,$4,(windowNode*)$5);
            }
        ;
operatorNode.selfdefine.body.init:
            /*empty*/   {$$=NULL;}
...
operator.selfdefine.body.window.list:
        ...
        |   WINDOW lblock operator.selfdefine.window.list rblock {
                line("Line:%-4d",@1.first_line);
                debug("operator.selfdefinr.body::=WINDOW {operator.selfdefine.window.list}\n");
                $$=new windowNode($3);
        }
        ;
...
operator.selfdefine.window:
            ...
            line("Line:%-4d",@1.first_line);
                debug("operator.selfdefinr.window::=%s window.type (sliding? (arg.list?))\n",$1->c_str());
